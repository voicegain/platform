...
def process_audio(file_name):
  ...
  print("downloading transcriptions", flush=True)

  time.sleep(2)
  print("done sleeping", flush=True)
  
  url = "{}://{}/{}/asr/transcribe/{}/transcript".format(protocol, hostPort, urlPrefix, session_id_left)
  print(f"making GET request to {url}", flush=True)
  left_script = requests.get(url, json=body, headers=headers)
  # download text transcript
  with open(f"{file_name}-left.txt", "wt", encoding="utf-8") as f:
    f.write(left_script.text)
  print(f"downloaded transcript to {file_name}-left.txt", flush=True)

  url = "{}://{}/{}/asr/transcribe/{}/transcript".format(protocol, hostPort, urlPrefix, session_id_right)
  print(f"making GET request to {url}", flush=True)
  right_script = requests.get(url, json=body, headers=headers, params={"format": "text"})
  # download text transcript
  with open(f"{file_name}-right.txt", "wt", encoding="utf-8") as f:
    f.write(right_script.text)
  print(f"downloaded transcript to {file_name}-right.txt", flush=True)
