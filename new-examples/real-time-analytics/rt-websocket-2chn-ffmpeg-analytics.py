...
def web_api_request_sa_config(headers, body):
  ...
  url = "{}://{}/{}/sa/config".format(protocol, hostPort, urlPrefix)
  ...
  if("BAD_REQUEST" == init_response.get("status") and  "the specified name is being used" in  init_response.get("message")):
    ...
    url = "{}://{}/{}/sa/config?name={}".format(protocol, hostPort, urlPrefix, sa_config_name)
    init_response_raw = requests.get(url, headers=headers)
    ...
    if(existing_sa_config_id is None):
      print(f"did not find existing SA config {sa_config_name} in current context")
      exit()
    ## delete old config
    print("delete old SA config: "+existing_sa_config_id)
    url = "{}://{}/{}/sa/config/{}".format(protocol, hostPort, urlPrefix, existing_sa_config_id)
    init_response_raw = requests.delete(url, headers=headers)
...
def web_api_request(headers, body):
  ...
  url = "{}://{}/{}/sa".format(protocol, hostPort, urlPrefix)
  init_response_raw = requests.post(url, json=body, headers=headers)
  init_response = init_response_raw.json()
...
def process_ws_msg(wsMsg):
    print("ws msg json: "+wsMsg, flush=True)
    audio_pos = 0
    if(epoch_start_audio_stream is not None):
        now = time.time()
        print("now: "+str(now)+" start: "+str(epoch_start_audio_stream), flush=True)
        audio_pos = int(round(1000.0*(now - epoch_start_audio_stream)))
        
    print("audio_pos: "+str(audio_pos), flush=True)
  ...
  with open(conv_fname, "rb") as f:
    async with websockets.connect(audio_ws_url, 
      ...
      try:
        ...
        print(f"start: {epoch_start_audio_stream}", flush=True)
        elapsed_time_fl = 0
        count = 0
        while byte_buf:
          n = len(byte_buf)
