...
# ───────────  NEW ambiguity map  ────────────────────────────────
#
# key  = any spoken alias that could appear by itself
# value = list[ tuple(first, last, dept, dn) ]
#
amb_alias: dict[str, list[tuple[str, str, str, str]]] = defaultdict(list)

for first, last, dept, dn, *optional in people_rows:
    # optional will be [] (length 0) when you only have 4 columns
    # or [first_aliases, last_aliases] when the loader supplies them
    first_aliases = optional[0] if len(optional) > 0 else []
    last_aliases  = optional[1] if len(optional) > 1 else []

    # package the fields you’ll need later – here the first four are enough
    person = (first, last, dept, dn)

    # 1️⃣ core spoken forms
    for spoken in (first, last):
        amb_alias[normalize_spoken(spoken)].append(person)

    # 2️⃣ first-name aliases (“Scot”, “Scotty”…)
    for a in first_aliases:
        amb_alias[normalize_spoken(a)].append(person)

    # 3️⃣ last-name aliases (“Davies”, “Dayvis”…)
    for a in last_aliases:
        amb_alias[normalize_spoken(a)].append(person)

    # 4️⃣ last-name + dept (“Cameron in IT”)
    # 4️⃣  last-name + dept forms (“Cameron in IT”, “Cameron in IT HIS” …)
    if dept:
        dept_norm   = normalize_spoken(dept)
        short_form  = dept_norm.split()[0]                     # "IT" from "IT HIS"
        dep_variants = {dept_norm, short_form} | set(aliases_by_dept.get(dept_norm, []))

        # all ways the person might be referred to
        tmp_name_variants = [first, last] + first_aliases + last_aliases

        for dep_variant in dep_variants:
            for nm in tmp_name_variants:                           # ← NEW: first **and** last
                key = f"{nm} in {dep_variant}"
                amb_alias[normalize_spoken(key)].append(person)
...
## ───────────  Add rules for every ambiguous alias  ───────────────
def _make_tag(lst, include_dept=False):
    parts = []
    for fn, ln, dept, dn, *_ in lst:
        base = f"{fn} {ln}"
        if include_dept and dept:
            base += f" in {dept}"
        parts.append(f"{base} number {dn}")
    return " or ".join(parts)

for spoken_norm, persons in amb_alias.items():
    if len(persons) < 2:
        continue  # unique → no ambiguity

    # decide whether the tag should include department
    include_dept = all(p[2] for p in persons) and " in " in spoken_norm

    rid = f"amb_{sanitize(spoken_norm)}"
    rule = create_rule(rid)
    rule_body = [create_item(spoken_norm)]        # spoken form
    wrap_rule(rule, rule_body, _make_tag(persons, include_dept))
    grammar.append(rule)

    pr_item = ET.Element("item")
    pr_item.append(create_ruleref(rid))
    primary_oneof.append(pr_item)

    # DEBUG
    print(f"[DEBUG] ambiguous '{spoken_norm}' → {len(persons)} people → rule {rid}")
# ────────────────────────────────────────────────────────────────
...
#  Write output
# ------------------------------------------------------------------ #
with open("Smith_Falls_9.grxml", "wb") as fh:
